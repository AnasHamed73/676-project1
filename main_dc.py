...
from tensorflow.contrib.gan import eval as gan_eval
...
def calc_fid(real, generated, model):
...
def load_inception(use_gpu=False):
    ...
    # Remove linear activation layer by replacing it with buffer
    inception.fc = Buffer()
    inception.eval()
    if use_gpu:
        inception.cuda()
...
def load_dataset():
    return dset.CIFAR10(root=opt.dataroot, download=True,
...
def train(netD, netG, dataloader, loss, optimizerD, optimizerG, calculate_fid=False):
    fixed_noise = torch.randn(opt.batchSize, nz, 1, 1, device=device)
    real_label = 1
    fake_label = 0
    if calculate_fid:
        inception = load_inception(opt.cuda)
    for epoch in range(opt.niter):
        for i, data in enumerate(dataloader, 0):
            ...
            if i % 100 == 0:
                ...
                if calculate_fid:
                    calc_fid(real_cpu.detach(), fake.detach(), inception)
...
train(netD, netG, dataloader, criterion, optimizerD, optimizerG, calculate_fid=False)
